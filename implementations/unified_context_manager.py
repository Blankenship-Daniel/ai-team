...
```bash
# ALWAYS use --dangerously-skip-permissions to prevent getting stuck on prompts
...
""",
    ...
    def __init__(self, install_dir: Optional[Path] = None):
        ...
        logger.info(
            f"UnifiedContextManager initialized",
            extra={"install_dir": str(self.install_dir), "working_dir": str(self.working_dir)},
        )
    ...
    def _inject_context_impl(self, original_briefing: str, role: str, environment: Dict[str, str]) -> str:
        ...
        # Add environment information - SECURITY: Properly escape directory paths
        escaped_working_dir = shlex.quote(str(self.working_dir))
        escaped_install_dir = shlex.quote(str(self.install_dir))

        env_context = f"""
### Current Environment
- Working Directory: {escaped_working_dir}
- Orchestrator Installation: {escaped_install_dir}
    ...
    def cleanup_workspaces(self, session_name: Optional[str] = None):
        ...
        if session_name:
            ...
            if session_workspace.exists():
                ...
                # Remove from cache
                self.workspaces = {k: v for k, v in self.workspaces.items() if not k.startswith(f"{session_name}:")}
        else:
