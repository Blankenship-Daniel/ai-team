...
class ContextRetentionManager:
    ...
    def dump_context(
        self, session_name: str, window_index: int, content: str, metadata: Optional[Dict] = None
    ) -> Optional[str]:
    ...
    def restore_context(self, session_name: str, window_index: int, hours_back: int = 24) -> Optional[str]:
            ...
            try:
                ...
                for session in sessions:
                    for window in session.windows:
                        content = orchestrator.capture_window_content(session.name, window.window_index, num_lines=100)
    ...
    def _check_communication_tools(self) -> bool:
        ...
        tools = ["send-claude-message.sh", "claude-write-file.sh", "notify-orchestrator.sh"]
...
"""
    ...
    def _find_duplicate(self, session_name: str, window_index: int, content_hash: str) -> Optional[str]:
        """Check if identical content already exists"""
        pattern = f"{session_name}_{window_index}_*.json"
