...
"""
    ...
    def build_agent_context(self, role: str, include_external: bool = True) -> str:
        ...
        logger.info(f"Building context for role: {role}", extra={
        ...
        context_parts = [
            f"## CONTEXT VERSION: {self.CONTEXT_VERSION}\n",
            self.CORE_CONTEXT
        ]
        ...
        context_parts.append(f"- Context Preserved: Yes (embedded)")
        
        full_context = "\n\n".join(context_parts)
        logger.info(f"Successfully built context for {role}", extra={
            'role': role,
...
CONTEXT VERSION: {self.CONTEXT_VERSION} - If you see version mismatch errors, run: ./restore_context.sh"""
            ...
            logger.info(f"Context injection successful for {role}", extra={
                'role': role,
            ...
            logger.error(f"Context injection failed for {role}: {e}", extra={
                'role': role,
                'error': str(e),
                'context_version': self.CONTEXT_VERSION
            })
...
You may be missing critical operational knowledge. Run: ./restore_context.sh"""
    ...
    def create_context_file(self, target_dir: Path = None) -> Path:
        ...
        try:
            ...
            logger.info(f"Created context file", extra={
            ...
            logger.error(f"Failed to create context file: {e}", extra={
                'path': str(context_file),
                'error': str(e)
            })
            raise
        ...
        if context_file.exists():
            try:
                ...
                if not cls.is_version_compatible(file_version):
                    logger.warning(f"Context version mismatch", extra={
                        'file_version': file_version,
                        'current_version': cls.CONTEXT_VERSION,
                        'compatible': cls.COMPATIBLE_VERSIONS
                    })
                else:
                    logger.debug(f"Context file loaded successfully", extra={
                ...
                logger.error(f"Failed to load context file: {e}", extra={
                    'path': str(context_file),
                    'error': str(e)
                })
        else:
    ...
    def validate_agent_context(cls, agent_response: str) -> tuple[bool, str]:
        ...
        if not cls.is_version_compatible(agent_version):
            logger.error(f"Agent context version mismatch", extra={
...
"""
        ...
        try:
            ...
            logger.info(f"Created fallback script", extra={
            ...
            logger.error(f"Failed to create fallback script: {e}", extra={
                'path': str(script_path),
                'error': str(e)
            })
            raise
    ...
    def monitor_agent_context(self, session_name: str, agent_name: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.error(f"Failed to monitor agent context: {e}", extra={
