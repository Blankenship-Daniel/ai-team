...
def setup_logging(
    name: str, log_dir: Optional[str] = None, console_level: str = "INFO", file_level: str = "DEBUG"
) -> logging.Logger:
    """
...
def log_subprocess_call(logger: logging.Logger, cmd: list, result=None, error=None):
    ...
    if error:
        logger.error(
            f"Subprocess failed: {safe_cmd}",
            extra={"command": cmd, "error": str(error), "error_type": type(error).__name__},
        )
    else:
        logger.debug(
            f"Subprocess succeeded: {safe_cmd}",
            extra={
                "command": cmd,
