...
from ai_team.utils.chaos_prevention import get_chaos_manager, chaos_protected
...
class TeamOrchestrationManager:
    ...
    def create_team(self, team_name: str, agents: List[str], session_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not force:
                ...
                self.coordinator.send_inter_team_message(
                    "system", team_id, "shutdown_request", {"reason": "Team destruction requested"}
                )
            ...
            return {"team_id": team_id, "status": "destroyed", "timestamp": time.time()}
        ...
        try:
            ...
            return {
                ...
                "protection_status": {
                    ...
                    "active_bulkheads": sum(b["active_operations"] for b in chaos_status["bulkheads"].values()),
                },
                "timestamp": time.time(),
            }
    ...
    def synchronize_team_context(self, team_id: str, context_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            self.context_manager.dump_context(
                f"team_{team_id}", 0, json.dumps(context_data), {"sync_type": "inter_team"}
            )

            return {"team_id": team_id, "sync_result": sync_result, "status": "synchronized", "timestamp": time.time()}

        except Exception as e:
            logger.error(f"Context sync failed for team {team_id}: {e}")
            return {"error": str(e)}

    def send_team_message(self, from_team: str, to_team: str, message_type: str, payload: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "chaos_protection": {
                    "circuit_breakers": len(chaos_status["circuit_breakers"]),
                    "open_circuits": sum(
                        1 for cb in chaos_status["circuit_breakers"].values() if cb["state"] == "open"
                    ),
                ...
                "context_system": {"healthy": context_health["healthy"], "issues": context_health.get("issues", [])},
            }
    ...
    def emergency_shutdown(self, reason: str = "Emergency shutdown") -> Dict[str, Any]:
        ...
        return {"shutdown_reason": reason, "teams_shutdown": shutdown_results, "timestamp": time.time()}
    ...
    def _protected_tmux_creation(self, orchestrator) -> bool:
        ...
        try:
            return self.chaos_manager.bulkheads["tmux_operations"].execute(orchestrator.create_team)
        except Exception as e:
    ...
    def _protected_tmux_destruction(self, session_name: str) -> bool:
        ...
        def destroy_session():
            ...
            try:
                subprocess.run(["tmux", "kill-session", "-t", session_name], check=True, timeout=10)
                return True
